...
planRouter.get("/current", async (req: Request, res: Response) => {
  try {
    ...
    if (!plan) {
      return res.status(404).json({ error: "No training plan found for this user" });
    }
    const parsedPlan = typeof plan.plan_json === 'string' ? JSON.parse(plan.plan_json) : plan.plan_json;
